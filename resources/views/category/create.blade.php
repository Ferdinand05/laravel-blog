<x-app-layout title="Create Category">
    <x-container>
        <div class="flex items-center gap-x-10 my-9 justify-between ">
            <div>
                <h1 class="font-bold text-3xl dark:text-gray-300">Create Categories</h1>
                <p class="dark:text-gray-300 md:text-start">Category for your Blog.</p>
            </div>
            <a href="{{ route('categories.index') }}"
                class="bg-red-600 hover:bg-red-700 text-white rounded py-1 px-3">Back</a>
        </div>
        <form class=" max-w-lg" action="{{ route('categories.store') }}" method="POST">
            @csrf
